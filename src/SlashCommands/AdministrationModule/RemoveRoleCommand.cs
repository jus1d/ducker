...
{
    [SlashCommand("remove-role", "Removes role from mentioned member")]
    [RequireAdmin]
    public async Task RemoveRoleCommand(InteractionContext msg,
        [Option("member", "Member for remove role")]
        DiscordUser user,
        [Option("role", "Role to remove it")] DiscordRole role,
        [Option("reason", "Reason to remove role")]
        string reason = "noneReason")
    {
        ...
        try
        {
            ...
            await Log.Audit(msg.Guild, $"{msg.Member.Mention} removed role {role.Mention} from {member.Mention}.",
                reason);
        }
        catch
        {
