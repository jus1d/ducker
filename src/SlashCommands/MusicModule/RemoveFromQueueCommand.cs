...
public partial class MusicSlashCommands
{
    ...
    [RequireMusicChannel]
    public async Task RemoveFromQueueCommand(InteractionContext msg,
        [Option("position", "Track's position in queue")] string positionInput)
    {
        await msg.CreateResponseAsync(DiscordEmoji.FromName(msg.Client, Bot.RespondEmojiName));

        if (!int.TryParse(positionInput, out var position))
        {
