...
public partial class MiscCommands
{
    ...
    public async Task ReportCommand(CommandContext msg, DiscordMember member,
    ...
    public async Task ReportCommand(CommandContext msg, [RemainingText] string reason = "No reason given")
    {
        ...
        await msg.Message.CreateReactionAsync(DiscordEmoji.FromName(msg.Client, Bot.RespondEmojiName));
        await Log.Audit(msg.Guild, $"{msg.User.Mention} reported {msg.Message.ReferencedMessage.Author.Mention},",
            reason, LogType.Report);
    }
}