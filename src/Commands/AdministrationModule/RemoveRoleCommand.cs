...
public partial class AdministrationCommands
{
    ...
    [RequireAdmin]
    public async Task RemoveRoleCommand(CommandContext msg, DiscordMember member, DiscordRole role,
        [RemainingText] string reason = "noneReason")
    {
        ...
        try
        {
            ...
            await Log.Audit(msg.Guild, $"{msg.Member.Mention} remove role {role.Mention} from {member.Mention}.",
                reason);
        }
        catch
        {
    ...
    {
        await msg.Channel.SendMessageAsync(Embed.IncorrectCommand(msg, "remove <member> <role> <reason>"));
    }
}